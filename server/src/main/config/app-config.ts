...
function loadAwsEnvs() {
  ...
  const awsEnvSchema = z.object({
    AWS_COGNITO_USER_POOL_ID: isTest ? z.string().default('test-pool-id') : z.string(),
    AWS_COGNITO_CLIENT_SECRET: isTest ? z.string().default('test-client-secret') : z.string(),
    AWS_COGNITO_CLIENT_ID: isTest ? z.string().default('test-client-id') : z.string(),
